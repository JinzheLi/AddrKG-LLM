...
class AddressRetriever:
    def __init__(self, args, device="cuda"):
        ...
        self.name_set = list({item['name'] for item in self.dataset})
    ...
    def _extract_candidates(self, address):
        ...
        matched_name = [n for n in self.name_set if n in address]
        return matched_districts, matched_streets, matched_name

    def search(self, query_text, top_k=5):
        # ✅ 自动解析可能的 district 和 street
        matched_districts, matched_streets, matched_name = self._extract_candidates(query_text)
        ...
        # ✅ 初步检索
        D, I = self.index.search(query_emb, top_k * 10)  # 先多取，再加权过滤
        results = []
